...
struct job_manifest_socket *job_manifest_socket_new() {
    ...
    jms = static_cast<job_manifest_socket *>(calloc(1, sizeof(*jms)));
    if (!jms)
        return NULL;
...
int job_manifest_socket_get_port(struct job_manifest_socket *jms) {
    ...
    /* TODO: detect the protocol by looking at sock_family, and pass to
     * getservbyname() */
    ...
    log_debug("converted service name '%s' to port %d", jms->sock_service_name,
              jms->port);
    return 0;
}
...
void setup_socket_activation(int kqfd) {
    ...
    EV_SET(&kev, socket_kqfd, EVFILT_READ, EV_ADD, 0, 0,
           reinterpret_cast<void *>(&setup_socket_activation));
